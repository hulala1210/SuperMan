...
//    }
    ...
    [GameDateService captureData:[self data] url:[self request].URL];
    
//    if (cacheModel) {
//        NSString *cacheStringkey = [[[self.request URL] absoluteString] cd_md5HexDigest];
//        [[SCYLoanHTMLCache defaultcache] removeObjectForKey:cacheStringkey];
//    }
//
//    [self cacheDataWithResponse:self.response redirectRequest:nil];
}
...
- (void)loadCacheData:(SCYWebViewCacheModel *)cacheModel {
    if (cacheModel) {
        ...
        printf("--- %s \n",[urlStr UTF8String]);
        NSLog(@"loadCacheDataUlrStr = %@",urlStr);
